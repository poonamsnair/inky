...
The source frame is a lighthouse and may provide extracted coordinate scaffolding: use it to understand pose, body contact, limb direction, and composition. Do not fly blind, but also do not paste or preserve the source pixels as the final artwork.
...
3. **Coordinate pass**: when exact silhouettes or props matter, call `window.inky.extractPathsFromImage()` in preview and choose which path groups become construction scaffolds.
4. **Guide-mark pass**: identify source marks that are only storyboard instructions: panel numbers, borders, arrows, long path lines, ghost poses, labels, or construction marks. Remove them from final art unless they are redrawn as short local accents.
5. **Anchor pass**: place head, neck/shoulder bridge, torso or body mass, primary prop, surface line, and framing.
6. **Attachment pass**: draw each connected chain before detail, for example head -> neck -> shoulders/body, or shoulder -> sleeve -> forearm -> wrist -> hand -> face.
7. **Silhouette pass**: make hands, mouth, facial details, head/hair, clothing, and props readable as flat simple shapes.
8. **Thumbnail pass**: view the frame small. If a shape cannot be named instantly, return to the lighthouse and redraw before detail.
9. **Ink pass**: add contours and important facial/body lines.
10. **Watercolor pass**: add color washes.
11. **Texture pass**: add hatching and handmade marks last.
...
- Extracted path groups used, if any, and why.
