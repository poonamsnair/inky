...
- First try the Canvas API preset shelf: `createBrush({ type: "pencil" })`, `charcoal`, `crayon`, or `watercolor`, using `listBrushes()` to confirm available names.
- Tune `textureScale`, `color`, `roughness`, `opacity`, and pressure settings before adding older specialist helpers.
...
Use `src/inky-canvas.js` first for new renderer work: `createBrush()`, `listBrushes()`, `hatch()`, `keyframe()`, and `timeline()`. Use `window.inky.extractPathsFromImage()` when reference contours need exact coordinates before redraw.

Use `src/material-tools.js` only when implementing or revising specialist material marks that the Canvas API brush shelf cannot express. Use `src/illustration-tools.js` when repeated body-chain, prop, or paper-scene helpers save real work.
...
- `roughjs` only through `drawRoughShape()` in `src/companion-tools.js` for safe non-semantic background accents. Do not use it for speech bubbles, labels, body silhouettes, hands, faces, borders, or contact-critical props.
...
When choosing or changing default brush presets, render the Canvas brush contact sheet:

```bash
npm run canvas:brushes
```

Approve a brush update only when the preview shows visibly different pencil, charcoal, crayon, and watercolor behavior. Specialist material helper changes need project-specific visual review in the browser.
