...
function resolveProjectDir(projectArg) {
  const candidate = resolve(root, projectArg);
...
async function inspectInBrowser(report, manifest, browserOptions) {
  ...
  try {
    ...
    await page.goto(projectUrl(baseUrl, manifest.slug, { preview: true }), { waitUntil: "networkidle" });
    await page.waitForFunction(() => window.storyboardApp?.hasLoadedAnimation === true, null, { timeout: 8000 });

    await page.goto(projectUrl(baseUrl, manifest.slug, { exportMode: true, frame: 0 }), { waitUntil: "networkidle" });
