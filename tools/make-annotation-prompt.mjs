...
import { projectPathForDir, projectRefForDir, resolveProjectDir as resolveProjectDirectory } from "./project-paths.mjs";
...
function buildPrompt({ projectDir, projectSlug, projectRef, projectPath, projectTitle, manifest, annotations, annotationData, annotationsPath, inputLabel }) {
  ...
  const lines = [
    ...
    "",
    `- \`http://127.0.0.1:5176/?project=${projectRef}\``,
    "",
    "## Active Annotations",
    "",
  ];
  ...
  annotations.forEach((annotation, index) => {
    ...
    if (annotation.screenshots.length) {
      lines.push(
        `- Current-frame screenshot${annotation.screenshots.length === 1 ? "" : "s"}: ${annotation.screenshots
          .map((item) => `\`${projectAssetPath(projectPath, item)}\``)
          .join(", ")}`,
      );
    }
    if (annotation.references.length) {
      lines.push(
        `- Reference image${annotation.references.length === 1 ? "" : "s"}: ${annotation.references
          .map((reference) => `\`${projectAssetPath(projectPath, reference.path)}\``)
          .join(", ")}`,
      );
    }
...
function projectAssetPath(projectPath, assetPath) {
  const value = String(assetPath || "").trim();
  if (!value || value.startsWith("data:") || /^[a-z]+:\/\//i.test(value) || path.isAbsolute(value)) return value;
...
function printUsage() {
  ...
  npm run storyboard:annotation-prompt -- <project-ref> -- --input storyboard/annotations.json --print
  npm run storyboard:annotation-prompt -- <project-ref> -- --input - --output -
