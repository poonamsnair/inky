...
- Style inspiration folder: \`${join(dirname(outputDir), "insp")}\`
...
- Important contours can be extracted with \`window.inky.extractPathsFromImage()\` and redrawn with Inky brushes when alignment matters.
...
2. Extract contour paths in preview when the source layout needs exact coordinates.
3. Draw frame 1 as a finished still from its lighthouse reference and extracted scaffolds.
4. For each next frame, draw from its own lighthouse reference and extracted scaffolds when useful.
5. Compare the new finished frame against the previous finished frame.
6. Fix semantic clarity and consistency drift before animating.
7. Animate only after still frames are coherent.
